...
- Show app-level acknowledgment messages through `source/AppMessage.bs` (`AppMessage.Show`, `AppMessage.Dismiss`). MainScene owns message state and return focus; a dedicated top-level OverlayHost presents MessageDialog without replacing an underlying feature dialog. Use these helpers only from component scripts because they depend on `m.top`. Keep progress in Spinner and empty states within their owning page. Dismiss messages during navigation or explicit retries, never from unrelated successful background responses.
