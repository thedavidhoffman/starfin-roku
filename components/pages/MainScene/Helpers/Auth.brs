...
' authBeginBlockingRequest
'-------------------------------------------------------------------------------
sub authBeginBlockingRequest()
    m.loadingSpinner.blockInteraction = true
    Spinner_Show(0)
    m.loadingSpinner.setFocus(true)
end sub

'-------------------------------------------------------------------------------
' authEndBlockingRequest
'-------------------------------------------------------------------------------
sub authEndBlockingRequest()
    m.loadingSpinner.blockInteraction = false
    Spinner_Hide()
end sub
...
sub authRefreshLoginSavedAccounts()
    server = Url_NormalizeServer(m.login.serverValue)
...
sub authHandleAccountPickerClosed(closed as object)
    ...
    source = "header"
    if request <> invalid then source = SafeString(request.source, source)
    ...
    if isAuthenticatedActiveAccount then isAuthenticatedActiveAccount = SafeString(m.session.token, "") <> ""
    if isAuthenticatedActiveAccount and SafeString(account.accountKey, "") = SafeString(m.session.accountKey, "") then
        restoreAccountPickerFocus(source)
        return
    end if
...
function authResolveRequestAccountKey(request as dynamic) as string
    ...
    requestToken = SafeString(request.token, "")
    sessionToken = SafeString(m.session.token, "")
    if requestToken = "" or requestToken <> sessionToken then return ""

    return SafeString(m.session.accountKey, "")
end function
