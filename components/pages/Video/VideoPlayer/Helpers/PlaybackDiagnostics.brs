...
sub logPlaybackContentAssignment(response as dynamic, startPosition as dynamic)
    ...
    m.log.writeDisplaySafe("Playback content assignment method=" + SafeString(response.playbackIdentity.playMethod, "") + " format=" + SafeString(response.streamFormat, "") + " startPosition=" + SafeString(startPosition, ""))
end sub
...
sub logPlaybackSeekRequest(reason as string, targetPosition as dynamic)
    ...
    m.log.writeDisplaySafe("Playback seek requested reason=" + reason + " from=" + SafeString(sourcePosition, "") + " to=" + SafeString(target, "") + " delta=" + SafeString(target - sourcePosition, "") + " state=" + LCase(SafeString(m.videoPlayer.state, "")) + " itemId=" + m.session.itemId + " playSessionId=" + m.session.playSessionId)
end sub
