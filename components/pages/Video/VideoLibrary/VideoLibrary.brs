...
'-------------------------------------------------------------------------------
sub initReferences()
    m.log = CreateLogger("VideoLibrary")
...
' runVideoLibraryTask
'-------------------------------------------------------------------------------
sub runVideoLibraryTask(request as object)
...
function getVisibleVideoLibraryItems() as object
    ...
    return getSortedVideoLibraryItems(m.pageState.allItems)
end function
...
sub showDecadeFilterRow()
    ...
    applyGridLayout(m.pageState.imageAspect)
    if m.pageState.selectedDecade >= 0 or m.pageState.renderDecadeFilterOnApply = true then renderItems(getVisibleVideoLibraryItems())
    m.pageState.renderDecadeFilterOnApply = false
end sub
...
sub showGenreFilterRow()
    ...
    m.filterButtonRow.visible = options.Count() > 0
    applyGridLayout(m.pageState.imageAspect)
    if m.pageState.selectedGenre <> "" then renderItems(getVisibleVideoLibraryItems())
end sub
...
sub advanceVideoLibraryProgressForKey(key as string)
    ...
    if focusedIndex = invalid or focusedIndex < 0 then focusedIndex = getCurrentVideoLibraryFocusedIndex()
...
sub updateVideoLibraryProgressLayout(imageAspect as string)
    if imageAspect <> "poster" then stopVideoLibraryProgressHold()

    m.videoLibraryProgressIndicator.layoutMode = imageAspect
    m.videoLibraryProgressIndicator.gridLeft = Number_ToInteger(m.itemsGrid.translation[0])
    m.videoLibraryProgressIndicator.gridTop = Number_ToInteger(m.itemsGrid.translation[1])
    m.videoLibraryProgressIndicator.itemWidth = Number_ToInteger(m.itemsGrid.itemSize[0])
    m.videoLibraryProgressIndicator.itemSpacingX = Number_ToInteger(m.itemsGrid.itemSpacing[0])
    m.videoLibraryProgressIndicator.numColumns = Number_ToInteger(m.itemsGrid.numColumns)
    m.videoLibraryProgressIndicator.numRows = Number_ToInteger(m.itemsGrid.numRows)
