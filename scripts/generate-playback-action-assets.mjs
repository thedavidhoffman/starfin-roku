...
for (const [profile, size] of [["fhd", 64], ["hd", 43]]) {
  await mkdir(`images/buttons/${profile}`, { recursive: true });
  for (const focused of [false, true]) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}"><rect width="${size}" height="${size}" rx="${size/2}" fill="${focused ? "white" : "#e1e8f7"}"/></svg>`;
    const inner = await sharp(Buffer.from(svg)).ensureAlpha().raw().toBuffer();
    if (!focused) for (let i = 3; i < inner.length; i += 4) inner[i] = Math.round(inner[i] * 34 / 255);
    const edge = size + 2;
    ...
    await writeFile(`images/buttons/${profile}/playback-action-${focused ? "focused" : "unfocused"}.9.png`, png);
  }
}
