...
VideoPlayer owns a startup-only artwork group above Video and below playback
controls. It shows a fullscreen cropped backdrop and an aspect-fitted logo at
[96,64], bounded by 600x220. Missing or failed logos use white title text; missing
or failed backdrops expose black. Episodes use their own primary image as the background, with no series-backdrop
fallback. Their logo still comes from the series or inherited logo references. Other titles use their own images. Existing request
data appears immediately and accepted playback-info responses can enrich it;
image loading never blocks playback and adds no metadata requests.

The shared spinner retains its fullscreen dimming. A separate upper-left black
fade improves logo contrast, generated by scripts/generate-playback-startup-gradient.mjs.
The artwork hides on first playing, stop, failure, or removal and releases its
image URIs. Startup retries may show it again, but same-title recovery and stream
restarts after playing do not. New titles reset the presentation. Later buffering
and seeking keep the normal video presentation.
