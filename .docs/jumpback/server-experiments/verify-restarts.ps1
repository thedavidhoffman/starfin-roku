...
$plans = Get-Content -Raw (Join-Path $PSScriptRoot 'restart-plans.json') | ConvertFrom-Json
$results = [System.Collections.Generic.List[object]]::new()
foreach ($container in @('mpegts', 'fmp4')) {
    foreach ($requested in @(991, 1009, 1011)) {
        ...
        & $ffmpeg -hide_banner -loglevel error -nostdin -ss $seek -fflags +genpts -i $SourceFile -map 0:0 -map 0:1 -c:v copy -start_at_zero -c:a libmp3lame -ac 2 -ab 256000 -af volume=2 -copyts -avoid_negative_ts disabled -max_muxing_queue_size 2048 -f hls -max_delay 5000000 -hls_time 6 -hls_segment_type $container @formatArgs -start_number $plan.SegmentIndex -hls_segment_filename "$dir/%d.$ext" -hls_playlist_type vod -hls_list_size 0 "$dir/out.m3u8" 2> "$dir/ffmpeg.log"
